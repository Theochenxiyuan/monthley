export default {
  app: {
    title: 'Monthley',
    brand: 'Monthley',
  },
  common: {
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    add: 'Add',
    close: 'Close',
    remove: 'Remove',
    reset: 'Reset',
    select: 'Select',
    input: 'Input',
    entry: 'Plan',
    confirm: 'Confirm',
    none: 'None',
  },
  unit: {
    count: '',
  },
  navigation: {
    timeline: 'Timeline',
    stats: 'Stats',
    settings: 'Settings',
  },
  timeline: {
    title: 'Timeline',
    month: 'Month',
    addEntry: 'Add Plan',
    noEntries: 'No plans',
    updatedAt: 'Updated at',
    topReached: 'This is the start of the timeline',
    bottomReached: 'This is the end of the timeline',
    current: 'Current',
    noRecord: 'No record',
    loadEarlier: 'Load earlier months',
    loadLater: 'Load later months',
  },
  entry: {
    name: 'Name',
    date: 'Date',
    category: 'Category',
    categoryPlaceholder: 'Type',
    namePlaceholder: 'Name',
    previousMonth: 'Previous month',
    nextMonth: 'Next month',
    unscheduled: 'No month yet',
    unscheduledHint: 'Keep it in To Schedule and place it later',
    validationError: 'Please fill in all plan information',
    status: 'Status',
    low: 'Low',
    medium: 'Medium',
    high: 'High',
    types: {
      learn: 'Learn',
      play: 'Play',
      watch: 'Watch',
      read: 'Read',
    },
    statuses: {
      not_started: 'To Do',
      in_progress: 'Doing',
      completed: 'Done',
    },
    shortTypes: {
      learn: 'Learn',
      play: 'Play',
      watch: 'Watch',
      read: 'Read',
    },
  },
  settings: {
    title: 'Settings',
    preferences: 'Preferences',
    backupRestore: 'Backup & Restore',
    dangerZone: 'Danger Zone',
    language: 'Language',
    appearance: 'Appearance',
    theme: 'Theme',
    light: 'Light',
    dark: 'Dark',
    system: 'System',
    expandAll: 'Always expand all months',
    expandAllTooltip:
      'If not selected, months with all plans completed in the past and all plans not started in the future will be collapsed by default, showing condensed text',
    quickSettings: 'Quick Settings',
    fullSettings: 'Open Full Settings',
    exportTimeline: 'Export Timeline',
    importTimeline: 'Import Timeline',
    exportSuccess: 'Export successful',
    exportError: 'Export failed',
    importSuccess: 'Import successful',
    importSuccessWithCount: 'Import successful, restored {count} plan(s)',
    importNoEntriesRestored: 'Import completed, but no plans were restored ({count} plan(s) in the file)',
    importError: 'Import failed',
    confirmImport: 'Import will merge with existing data, continue?',
    importWarning: 'Note: Import will merge with existing data, not overwrite',
    invalidFileType: 'Please select a JSON file',
    clearTimeline: 'Clear Entire Timeline',
    confirmClear: 'This will permanently delete all timeline data and cannot be undone! Continue?',
    clearSuccess: 'Timeline cleared',
    clearError: 'Failed to clear timeline',
  },
  filters: {
    title: 'Filters',
    categories: 'Categories',
    status: 'Status',
    notes: 'Note: Filters will expand the months to show the filtered entries',
    dateRange: 'Date Range',
  },
  date: {
    formats: {
      month: 'MMMM yyyy',
      monthPicker: 'MMMM YYYY',
      date: 'MMMM d, yyyy',
      time: 'HH:mm:ss',
      dateTime: 'MMMM d, yyyy HH:mm:ss',
    },
    secondsAgo: '{count} seconds ago',
    minutesAgo: '{count} minutes ago',
    hoursAgo: '{count} hours ago',
    today: 'Today',
    yesterday: 'Yesterday',
    dayBeforeYesterday: 'Day before yesterday',
  },
  punctuation: {
    colon: ': ',
    semicolon: ';',
    comma: ',',
    period: '.',
    exclamation: '!',
    question: '?',
    dash: ' - ',
    ellipsis: '...',
    quote: {
      single: ["'", "'"],
      double: ['"', '"'],
    },
    bracket: {
      round: ['(', ')'],
      square: ['[', ']'],
      curly: ['{', '}'],
    },
    space: {
      betweenWords: ' ',
      betweenTypeAndName: ' ',
      betweenStatusAndType: ' ',
      listSeparator: ', ',
      typeSeparator: '; ',
    },
  },
  monthCard: {
    confirmDelete:
      'Are you sure you want to delete this plan? This action cannot be undone.',
    deleteEntry: 'Delete Plan',
    confirm: 'Confirm',
    cancel: 'Cancel',
    deleteSuccess: 'Deleted successfully',
    emptyMonth: 'Empty months will be automatically cleared after app restart',
    hiddenEntries: 'hidden',
    entries: 'plans',
    completed: 'Completed',
    inProgress: 'Doing',
    planned: 'Planned',
    andMore: 'and {count} more',
    start: 'Start',
    complete: 'Complete',
    moveToCurrentMonth: 'Move to current month',
    edit: 'Edit',
    delete: 'Delete',
    collapse: 'Collapse',
    incompletePlans: 'Unfinished Plans',
  },
  unscheduled: {
    title: 'To Schedule',
    add: 'Add unscheduled plan',
    resetLayout: 'Reset layout',
    resize: 'Resize',
    count: '{count} plan(s)',
    aiSchedule: 'AI Auto Schedule',
  },
  autoSchedule: {
    title: 'AI Schedule Plan',
    confirm: 'Confirm',
    cancel: 'Cancel',
    loading: 'AI is analyzing...',
    empty: 'No unscheduled plans',
    error: 'AI scheduling failed, please try again later',
    preConfirmTitle: 'AI Auto Schedule',
    preConfirmMessage: 'Your unscheduled plan names and notes will be sent to AI for analysis. AI will suggest target months based on content semantics and current timeline workload. This may take a few seconds.',
  },
  entryItem: {
    statusType: {
      not_started: {
        learn: 'To Learn',
        play: 'To Play',
        watch: 'To Watch',
        read: 'To Read',
      },
      in_progress: {
        learn: 'Learning',
        play: 'Playing',
        watch: 'Watching',
        read: 'Reading',
      },
      completed: {
        learn: 'Learned',
        play: 'Played',
        watch: 'Watched',
        read: 'Read',
      },
    },
  },
  stats: {
    title: 'Statistics',
    noData: 'No data yet. Start adding plans!',
    moreComing: 'More charts coming soon…',
    totalEntries: 'Total',
    completed: 'Completed',
    completionRate: 'Completion',
    inProgress: 'In Progress',
    notStarted: 'Not Started',
    totalMonths: 'Active Months',
    typeDistribution: 'Type Distribution',
    statusDistribution: 'Status Distribution',
    yearHeatmap: 'Year Heatmap',
    monthlyTrend: 'Monthly Trend',
    completionTrend: 'Completion Trend',
    streakMonths: 'Longest Streak',
    month: ' months',
    less: 'Less',
    more: 'More',
    noEntries: 'No entries',
  },
  search: {
    title: 'Search',
    placeholder: 'Search plans…',
    noResults: 'No matching plans found',
    resultCount: '{count} result(s)',
    hint: 'Type to search plans',
  },
  yearSummary: {
    title: 'Annual Review',
    heading: '{year} Annual Review',
    noData: 'No records for this year',
    aiTab: 'AI Summary',
    statsTab: 'Stats',
    aiEmptyTitle: 'Let AI write the echo of your year',
    aiEmptyDescription: 'Generate a warmer annual letter from this year\'s records, completion rhythm, and monthly patterns.',
    aiPrivacyNote: 'Generating sends this year\'s record summary to the AI service.',
    generateAi: 'Generate AI Summary',
    regenerateAi: 'Regenerate',
    aiGenerated: 'AI summary generated',
    aiFailed: 'AI summary failed',
    generatedAt: 'Generated at {time}',
    aiHighlights: 'Highlights',
    aiPatterns: 'Yearly Patterns',
    aiSuggestions: 'Suggestions for Next Year',
    aiStaleNote: 'Your records seem to have changed. Regenerate to refresh this summary.',
    copyAi: 'Copy Text',
    copySuccess: 'Summary copied',
    copyFailed: 'Copy failed',
    shareAiImage: 'Share Image',
    totalPlans: 'Total Plans',
    completionRate: 'Completion',
    activeMonths: 'Active Months',
    streakMonths: 'Longest Streak',
    topMonth: 'Most active: {month} with {count} plans',
    share: 'Share Year in Review',
  },
  syncOnboarding: {
    title: 'Enable Cloud Sync?',
    description: 'A sync key lets you restore and sync your timeline across devices. You can skip sync and keep data only on this device.',
    createNewKey: 'Create New Key',
    useExistingKey: 'Use Existing Key',
    skip: 'Skip for Now (Local Only)',
    existingDescription: 'Enter an existing sync key to restore the matching timeline from the cloud.',
    generatedDescription: 'Your new sync key is ready. Save it somewhere safe so you can use it on another device.',
    copySuccess: 'Key copied',
    copyFailed: 'Copy failed',
  },
  sync: {
    title: 'Cloud Sync',
    syncKey: 'Sync Key',
    generateKey: 'Generate New Key',
    confirmGenerateKey: 'After generating a new key, save it somewhere safe. If you already have a key, the new key will replace it and future syncs will use the new key. Continue?',
    syncNow: 'Sync Now',
    clearKey: 'Clear Key',
    useExistingKey: 'Use Existing Key',
    useOtherKey: 'Use Another Key',
    enterKeyPlaceholder: 'Paste sync key',
    showQrCode: 'Show QR Code',
    scanQrCode: 'Scan QR Code',
    qrTitle: 'Sync Key QR Code',
    qrSecurityWarning: 'Anyone who sees this QR code can use your sync key. Only show or scan it between trusted devices.',
    confirmShowQrCode: 'This QR code exposes your sync key. Make sure your surroundings are safe before showing it.',
    confirmImportKey: 'This will use the scanned sync key and try to sync from the cloud. Continue?',
    confirmReplaceKey: 'This will replace your current sync key and try to sync from the cloud. Continue?',
    invalidQrCode: 'This is not a valid Monthley sync key QR code',
    cameraUnavailable: 'Camera unavailable. Check browser permissions or device support.',
    scanQrHint: 'Place the Monthley sync key QR code inside the frame',
    importKeySuccess: 'Key imported and synced successfully',
    importKeyFailed: 'Key imported, but sync failed',
    copySuccess: 'Key copied',
    copyFailed: 'Copy failed',
    noKey: 'Please set a sync key first',
    syncSuccess: 'Sync successful',
    syncFailed: 'Sync failed',
    restoredFromCloud: 'Data restored from cloud',
    errorNotConfigured: 'Sync is not configured. Check Supabase environment variables.',
    errorCloudFileMissing: 'Cloud sync file was not found. The sync key may be invalid or the cloud data was deleted.',
    errorInProgress: 'Sync is already in progress. Please try again shortly.',
    errorNetwork: 'Network error. Sync failed, please try again later.',
    errorPermission: 'Sync permission denied. Check cloud storage policies or the sync key.',
    errorInvalidData: 'Cloud data cannot be decrypted or has an invalid format. Check the sync key.',
    errorPayloadTooLarge: 'Sync data is too large. Export a backup and clean old data.',
    statusTitle: 'Sync Status',
    localUpdated: 'Last Local Update',
    lastSynced: 'Last Synced',
    cloudUpdated: 'Latest Cloud Update',
    cloudStatus: 'Cloud Status',
    configured: 'Configured',
    notConfigured: 'Not Configured',
    syncNotEnabled: 'Sync not enabled',
    neverSynced: 'Never synced',
    notFetched: 'Not fetched yet',
  },
};
