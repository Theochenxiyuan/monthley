...
async function generateAiSummary() {
  ...
  const requestYear = currentYear.value;
  const requestCacheKey = cacheKey.value;
  const requestHash = currentInputHash.value;
  ...
  try {
    ...
    const saved: CachedAiYearSummary = {
      year: requestYear,
      generatedAt: new Date().toISOString(),
      inputHash: requestHash,
      result: data.data,
    };
    localStorage.setItem(requestCacheKey, JSON.stringify(saved));

    if (currentYear.value === requestYear) {
      aiSummary.value = saved;
    }
...
<template>
  <div class="year-summary-panel">
    <div class="summary-dialog-header year-summary-panel-header">
    ...
    <div class="year-summary-panel-body">
    ...
    <template v-else>
      ...
      <div v-if="yearData" class="summary-footer-actions year-summary-panel-footer">
...
.year-summary-panel--dialog .year-summary-panel-header {
  padding: 10px 12px 6px;
}

.year-summary-panel--dialog .year-summary-panel-body {
  padding: 0 8px 8px;
}

.year-summary-panel--dialog .year-summary-panel-footer {
  padding: 8px 10px 10px;
}

.year-summary-panel--dialog .summary-tab-switch {
  margin-bottom: 0.5rem;
}

.year-summary-panel--dialog .ai-empty-card,
.year-summary-panel--dialog .ai-summary-card,
.year-summary-panel--dialog .summary-card {
  padding: 0.9rem;
}

.year-summary-panel-footer {
  flex: 0 0 auto;
...
.ai-summary-card {
  ...
  --ai-card-text: #1f2933;
  --ai-card-border: rgba(92, 64, 38, 0.14);
  --ai-card-bg-gradient: linear-gradient(145deg, #fff9ef 0%, #f6efe3 58%, #efe3d1 100%);
  --ai-card-kicker-bg: rgba(255, 255, 255, 0.55);
  --ai-card-kicker-text: #8a5b1d;
  --ai-card-date: rgba(31, 41, 51, 0.58);
  --ai-section-border: rgba(92, 64, 38, 0.12);
  --ai-section-heading: #8a5b1d;

  color: var(--ai-card-text);
  border: 1px solid var(--ai-card-border);
  background: var(--ai-card-bg-gradient);
  box-shadow: 0 16px 40px rgba(84, 54, 28, 0.14);
}

:global(html.dark .ai-summary-card) {
  --ai-card-text: #e2e8f0;
  --ai-card-border: rgba(255, 255, 255, 0.1);
  --ai-card-bg-gradient: linear-gradient(145deg, #1a1a2e 0%, #16213e 58%, #0f172a 100%);
  --ai-card-kicker-bg: rgba(255, 255, 255, 0.1);
  --ai-card-kicker-text: #e2e8f0;
  --ai-card-date: rgba(226, 232, 240, 0.5);
  --ai-section-border: rgba(255, 255, 255, 0.08);
  --ai-section-heading: #94a3b8;
...
.ai-card-kicker {
  ...
  border-radius: 999px;
  background: var(--ai-card-kicker-bg);
  color: var(--ai-card-kicker-text);
  font-size: 0.68rem;
  font-weight: 800;
...
.ai-section {
  padding: 0.85rem 0;
  border-top: 1px solid var(--ai-section-border);
}

.ai-section h4 {
  margin: 0 0 0.5rem;
  color: var(--ai-section-heading);
  font-size: 0.86rem;
}
